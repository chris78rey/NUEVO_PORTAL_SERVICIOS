...
public class CuentasPlan implements Serializable {
    ...
    @OneToMany(cascade = CascadeType.ALL, mappedBy = "cuentasPlan", fetch = FetchType.LAZY)
    private List<Subcuentas> subcuentasList;
    @JoinColumns({
        @JoinColumn(name = "MYR_EMP_CODIGO", referencedColumnName = "EMP_CODIGO", insertable = false, updatable = false),
        @JoinColumn(name = "MYR_CODIGO", referencedColumnName = "CODIGO", insertable = false, updatable = false)})
    @ManyToOne(optional = false, fetch = FetchType.LAZY)
    private Mayores mayores;
    ...
    public List<Subcuentas> getSubcuentasList() {
        return subcuentasList;
    }

    public void setSubcuentasList(List<Subcuentas> subcuentasList) {
        this.subcuentasList = subcuentasList;
    }

    public Mayores getMayores() {
        return mayores;
    }

    public void setMayores(Mayores mayores) {
    ...
    public String toString() {
        return "ec.mil.he1.pom_02_domain.entities.CuentasPlan[ cuentasPlanPK=" + cuentasPlanPK + " ]";
    }
