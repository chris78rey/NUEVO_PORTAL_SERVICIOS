...
public class Subcuentas implements Serializable {
    ...
    @OneToMany(cascade = CascadeType.ALL, mappedBy = "subcuentas", fetch = FetchType.LAZY)
    private List<Auxiliares> auxiliaresList;
    @JoinColumns({
        @JoinColumn(name = "CNT_MYR_EMP_CODIGO", referencedColumnName = "MYR_EMP_CODIGO", insertable = false, updatable = false),
        @JoinColumn(name = "CNT_MYR_CODIGO", referencedColumnName = "MYR_CODIGO", insertable = false, updatable = false),
        @JoinColumn(name = "CNT_CODIGO", referencedColumnName = "CODIGO", insertable = false, updatable = false)})
    @ManyToOne(optional = false, fetch = FetchType.LAZY)
    private CuentasPlan cuentasPlan;
    ...
    public List<Auxiliares> getAuxiliaresList() {
        return auxiliaresList;
    }

    public void setAuxiliaresList(List<Auxiliares> auxiliaresList) {
        this.auxiliaresList = auxiliaresList;
    }

    public CuentasPlan getCuentasPlan() {
        return cuentasPlan;
    }

    public void setCuentasPlan(CuentasPlan cuentasPlan) {
        this.cuentasPlan = cuentasPlan;
    }
    ...
    public String toString() {
        return "ec.mil.he1.pom_02_domain.entities.Subcuentas[ subcuentasPK=" + subcuentasPK + " ]";
    }
